...
function partialApply(f, x) {
    if (fNames.indexOf(f.name) == -1) { 
      fNames.push(f.name);
    } else {
      fNames.move(fNames.indexOf(f.name), fNames.length-1);
    }
    return function () {
...
function moveEntry(arr, from, to) {
    arr.splice(to, 0, arr.splice(from, 1)[0]);
    return arr;
};

function partialApply(f, x) {
    /*if (f.name.search("fun") != -1) {
      if (fNames.indexOf(f.name) == -1) { 
        fNames.push(f.name);
      } else {
        fNames = moveEntry(fNames,fNames.indexOf(f.name),fNames.length-1);
      }
    } else {
      console.log("search failed: " + f.name);
    }*/
