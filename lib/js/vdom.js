...
function setupSubscriptions(subscriptions) {
  ...
  const evtName = subscription["_value"]["1"];
  const genMsgFn = subscription["_value"]["2"];
  const variantTag = subscription["_label"];

  if (variantTag == "VirtualDom.StringHandler") {
...
function _runDom(str, doc, pid, subscriptions) {
  evtHandlerPid = pid;
